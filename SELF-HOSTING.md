...
This document is for the owner of `pinheiroclimatizacao.example` (or any client who has been given a copy of this codebase). It explains, accurately, what's involved in hosting and maintaining the site yourself if you ever decide not to use the managed plan.
...
- Most static hosts ask for a **CNAME record** for the `www` subdomain, pointing to a hostname they provide (e.g. `pinheiroclimatizacao.netlify.app`)
- Some require an **A record** for the apex/root domain (`pinheiroclimatizacao.example`) pointing to a specific IP address
...
- A business email address (e.g., `info@pinheiroclimatizacao.example` instead of a Gmail address) for credibility — this requires either Google Workspace (~$6/user/month), Microsoft 365 (~$6/user/month), or a basic email-forwarding setup at your domain registrar (usually free, but with limitations)
...
If you're already using `pinheiroclimatizacao@gmail.com` as your business email, the form can deliver to that address, but Google's deliverability rules are strict and form-backend emails sometimes land in Gmail's Promotions or Spam folders. Test thoroughly.
